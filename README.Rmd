...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/ibdfindr)](https://CRAN.R-project.org/package=ibdfindr)
...
As an example we consider the built-in dataset `cousinsDemo`, which contains almost 4000 SNP genotypes for two related individuals. The data were simulated assuming a relationship of first cousins. (For complete details on how the data was generated, the source code is available in the `data-raw` folder.)
```{r, eval = F}
head(cousinsDemo)
```
```{r, echo = F}
